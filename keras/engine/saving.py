...
def _deserialize_model(f, custom_objects=None, compile=True):
    ...
    if 'keras_version' in model_weights_group:
        original_keras_version = model_weights_group['keras_version'].encode('utf-8').decode('utf8')
    else:
        original_keras_version = '1'
    if 'backend' in model_weights_group:
        original_backend = model_weights_group['backend'].encode('utf-8').decode('utf8')
    else:
    ...
    if compile:
        ...
        if training_config is None:
            warnings.warn('No training configuration found in save file: '
                          ...
                          'Compile it manually.')
            return model
        training_config = json.loads(training_config.encode('utf-8').decode('utf-8'))
